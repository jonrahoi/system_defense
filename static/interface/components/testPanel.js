...
TestPanel.prototype.buildObject = function() {
    ...
    this.addComponentBtn.clicks(() => {
        ...
        if (this.componentCount < 2){
          ...
          let name = 'IPHONE';
          this.controls.placeComponent(this.id, this.x, this.y, name);
          this.x += 20;
          this.y += 20; 
          this.componentCount++
          this.id = (this.id + 1) % 2  
        } else {
          console.log("components cant more than 2")
        }
    });
    ...
    this.removeComponentBtn.clicks(()=>{
      if (this.componentCount > 0){
        let iphones = k.get("IPHONE")
        iphones[0].destroy()
        this.componentCount--
      }
      this.controls.removeComponent()
    });
