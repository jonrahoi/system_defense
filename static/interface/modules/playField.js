...
export default function PlayField(screenX, screenY, screenWidth, screenHeight, level) {
    this.init(screenX, screenY, screenWidth, screenHeight);

    // Expose function anonymously to ensure correct context
    this.build = (levelObj) => { this.buildObject(levelObj); };
};
...
PlayField.prototype.init = function(screenX, screenY, screenWidth, screenHeight) {
    ...
    this.params = {
        ...
        levelSpace: {
            ...
            backgroundColor: k.color(176, 182, 221),
...
PlayField.prototype.buildObject = function(levelObj) {

    // Outer rectangle container
    k.add([ 
        k.rect(this.params.width, this.params.height - this.params.height * this.params.descripRatio),
        k.pos(this.params.x, this.params.y + this.params.height * this.params.descripRatio),
    ...
    k.add([
     ...
     k.add([ 
        k.rect(this.params.levelSpace.width, this.params.levelSpace.height),
        k.pos(this.params.levelSpace.x, this.params.levelSpace.y),
        k.outline(this.params.outlineWidth, this.params.outlineColor),
        this.params.levelSpace.backgroundColor,
        this.params.levelSpace.backgroundOpacity,
    ]);

    k.add([
        k.text(levelObj.specs.description, {
            size: this.params.height * this.params.descripRatio,
        }),
