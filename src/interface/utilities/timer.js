...
const Timer = {
    ...
    restart: function(duration) {
        ...
        Timer._executeCallbacks(TimerControls.RegistrationTypes.BASE_INTERVAL);
        Timer._executeCallbacks(TimerControls.RegistrationTypes.SPEEDUP_INTERVAL);
    },
    ...
    registerListener: function(listenerFunc, context, 
                            type=TimerControls.RegistrationTypes.BASE_INTERVAL) {
        
        if (!Object.values(TimerControls.RegistrationTypes).includes(type)) { return; }
    ...
    _intervalCallback: (() => {
        const callback = () => {
            ...
            if (--Timer._time < 0) {
                Timer.stop();
                Timer._executeCallbacks(TimerControls.RegistrationTypes.TIMEOUT);
                return;
            }
...
export const TimerControls = {
    ...
    RegistrationTypes: Object.freeze({
        BASE_INTERVAL: 'BASE',
...
// Ensure synchronization between controls and timer object
Timer._bindings = Object.assign({}, 
    ...Object.values(TimerControls.RegistrationTypes).map(x => ({ [x]: []})));
